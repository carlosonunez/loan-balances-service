  ...
  module AWSHelpers
    module APIGateway
      ...
      def self.get_endpoint(event, path_to_remove: '/auth')
        ...
        path = event['requestContext']['path'] || raise('Path not found.')
      ...
      def self.send_response(code:, payload:)
        if !payload.nil? && (payload.class != Hash)
          raise 'Payload must be a Hash'
        end
      ...
      private_class_method :return_422
      private_class_method :return_404
      private_class_method :return_403
      private_class_method :return_200
      private_class_method :send_response
    end
  end
end
